...
contract GoalTracker {
  mapping(bytes32 => bytes32[]) public goalDescriptions; // hopefully bytes32 fits
  mapping(bytes32 => bool[]) public goalCompleted;
  ...
  function addGoals(bytes32 _member, bytes32[] _description) public {
  ...
  function goals(bytes32 _member) public view returns (bytes32[], bool[]) {
  ...
  function completeGoal(bytes32 _member, bytes32 _description) public {
    ...
    for (uint256 i = 0; i < descriptions.length; i++) {
      if (descriptions[i] == _description) {
        ...
        if(checkGoals(goalCompleted[_member])) {
         familyWallet.getReceivingAddress(_member).transfer(award); 
        }
        break;
      }
    }
